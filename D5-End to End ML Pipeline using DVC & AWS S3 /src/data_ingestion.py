...
def load_data(data_url:str)-> pd.DataFrame:
    try:
        ...
        logger.debug("Data loadeded from %s",data_url)
        return df
        ...
        logger.error("Unexpected error occured durring processing : %s", str(e))
        raise
...
def preprocess_data(df:pd.DataFrame)->pd.DataFrame:
    try:
        ...
        logger.error("Missing columns in data frame:%s",str(e))
        raise
    
    except Exception as e:
        logger.error("Unexpected error occured dureing preprocessing: %s",str(e))
        raise
...
def save_data(train_data:pd.DataFrame,test_data:pd.DataFrame,data_path:str)->None:
    try:
        ...
        logger.error("Unexpected error occur while saving the file %s",str(e))
        raise
...
def main():
    try:
        ...
        logger.error("Failed to complete data ingestion process: %s",str(e))
        print(f"Error:{e}")
        # raise
