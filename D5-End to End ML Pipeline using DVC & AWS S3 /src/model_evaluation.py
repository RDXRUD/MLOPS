...
def load_model(file_path:str):
    try:
        ...
        logger.error("Unexpected error occur wwhile loading model: %s",e)
        raise
...
def evaluate_model(clf,X_test:np.ndarray,y_test:np.ndarray)->dict:
    try:
        ...
        logger.error("Unexpected error occured while model evaluation: %s",e)
        raise
...
def save_metrices(metrices:dict,file_path:str)->None:
    try:
        ...
        logger.debug("Metrics saved to %s",file)
        
    except Exception as e:
        logger.error("Unexpected error occure while saving metrices: %s",e)
        raise
